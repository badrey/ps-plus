...
import {DisplayPrice, RawPriceInfo, RODisplayPrice, ROPriceInfo} from "./types";
import {ResponseItem, SkuPrice, TitleInfo} from "../api/store-api-types";

function normalisePriceInfo(priceInfo: RawPriceInfo): ROPriceInfo | null | undefined {
    if (!priceInfo) {
        return null;
    }
...
): {end: string; isPlus: boolean; start: string} | null | undefined {
    if (!priceInfo) {
        return null;
    }
