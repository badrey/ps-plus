...
type TitleDetailsSceneProps = {
    ...NavigationProps,
};

export const TitleDetailsSceneBase = ({title}: {title: Title}) => {
    const scrollViewRef = React.useRef<React.ElementRef<typeof ScrollView>>();
    ...
    const onDescSectionLayout = React.useCallback(
        ({nativeEvent: {layout}}) => {
            if (scrollViewRef.current && scrollToDescSection.current) {
                scrollViewRef.current.scrollTo({x: 0, y: layout.y, animated: true});
            }
